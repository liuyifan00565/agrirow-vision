...
import math
import time
from ortho_trans import create_ortho_trans

DEFAULT_CAMERA_WIDTH = 640  #摄像头拍摄宽度分辨率
DEFAULT_CAMERA_HEIGHT = 480 #摄像头拍摄高度分辨率
real_frame_width = None
real_frame_height = None
...
class VideoThread(QThread):
    ...
    def start_capture(self, source, format,backend=cv2.CAP_DSHOW):
        ...
        try:
            ...
            fourcc1 = video_encoder[format]['fourcc1']
            print("fourcc1=",fourcc1)
            if fourcc1:
                print("fourcc1=",fourcc1)
                self.cap.set(cv2.CAP_PROP_FOURCC, fourcc1)

            print(f"当前采集格式代码: {int(self.cap.get(cv2.CAP_PROP_FOURCC))}")  # 1196444237 表示MJPG
            ...
            self.cap.set(cv2.CAP_PROP_FPS, self.frame_rate)  # 强制设置为30 FPS
            actual_fps = self.cap.get(cv2.CAP_PROP_FPS)
            print(f"🎥 录制分辨率: {real_frame_width}x{real_frame_height}")
            print(f"🎥 录制帧率: {actual_fps}")
    ...
    def run(self):
        ...
        frame_delay = 1000 / self.frame_rate  # 目标每帧耗时（ms）
        ...
        while self.running and self.cap and self.cap.isOpened():
            ...
            if not ret:
                continue
            self.frame_ready.emit(frame,seq)  # 发射帧数据
            seq = (seq+1) % self.frame_rate
            # self.msleep(int(1000 / self.frame_rate))
            # 动态休眠
            elapsed_ms = (time.perf_counter() - start_time) * 1000
            if elapsed_ms < frame_delay:
                self.msleep(int(frame_delay - elapsed_ms))
...
class OrthophotoApp(QMainWindow):
    def __init__(self,format,camera_index):
        super().__init__()
    ...
    def save_original_video(self):
        ...
        if self.recording:
            ...
            video_filename = f"original_video_{timestamp}.{video_encoder[self.format]['ext']}"
    ...
    def start_recording(self, output_path):
        ...
        try:
            ...
            fourcc2 = video_encoder[self.format]['fourcc2']
            fps = 30.0
            self.video_writer = cv2.VideoWriter(output_path, fourcc2, fps, (width, height))
        ...
        if file_path:
            ...
            if self.ortho_mode:
                try:
                    ...
                    frame = self.bird_eye_transformer.transform(frame)
                except Exception as e:
    ...
    def update_pitch_angle(self):
        try:
            ...
            #     bird_eye_img, _ = self.bird_eye_transformer.transform(self.current_frame)
    ...
    # 确保连接信号槽（取消注释）
        ...
        self.video_thread.frame_ready.connect(self.save_original_frame)
        self.video_thread.frame_ready.connect(self.update_frame)
        ...
        if self.video_source == "camera":
            ...
            for backend in backends:
                ...
                if self.video_thread.start_capture(self.camera_index, self.format,backend):
                    break
            else:
                QMessageBox.critical(self, "错误", "无法打开摄像头")
                return
        else:
            if not self.video_thread.start_capture(self.video_source,self.format):
    ...
    def process_display_queue(self):
        ...
        try:
            ...
            if self.ortho_mode and self.bird_eye_transformer:
                try:
                    display_frame= self.bird_eye_transformer.transform(frame)
    ...
    def update_transformer_from_inputs(self):
        try:
            ...
            self.bird_eye_transformer = create_ortho_trans(
                fov_x_deg=fov_x,
                fov_y_deg=fov_y,
                K=self.camera_matrix,
                dist=self.dist_coeffs,
                camera_img_shape=(real_frame_height,real_frame_width),
    ...
    def save_original_frame(self,frame,seq):
        if frame is None or frame.size == 0:
            return
        self.original_frame = frame.copy()
        if self.recording and self.video_writer:
    ...
    def update_frame(self, frame,seq):
        if frame is None or frame.size == 0:
            return
...
def main():
    ...
    """
    format='mp4',按照mp4v编码格式保存
    format='avi',按照MJPG编码格式保存
    """
    window = OrthophotoApp(format='avi',camera_index = 1)
    window.show()
    sys.exit(app.exec_())
