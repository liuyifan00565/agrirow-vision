...
# 要是所有耗时按钮都交给线程的话，可以开个线程池，那四个摄像头优先级比较高QThreadPool  QtConcurrent
...
# thread1.py (只修改 CameraThread1 部分)

class CameraThread1(QThread):
    image_data = pyqtSignal(QImage)
    plot_signal = pyqtSignal(list, list)  # 新增：为 matplotlib 绘图传递数据
    ...
    def run(self):
        ...
        while self.working:
            try:
                ...
                # 接收图像
                pred = recv_img(client, 230400, [240, 320, 3])
                if pred is not None:
                    img = QtGui.QImage(pred.data, pred.shape[1], pred.shape[0], QImage.Format_BGR888)
                    if img.isNull():
...
class CameraThread2(QThread):
    ...
    def run(self):
        ...
        while self.cap.isOpened():
            ...
            img = QtGui.QImage(frame.data, frame.shape[1], frame.shape[0], QImage.Format_BGR888)
            self.image_data.emit(img)
