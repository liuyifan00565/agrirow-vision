...
from .Ui_parameter import Ui_Form  # 导入参数设置窗口
from .cutter_window import CutterParameterWindow  # 导入刀具参数窗口
from .crop_window2 import CropParameterWindow  # 导入作物参数窗口
from .DemoBoard_window import DemoBoardParameterWindow  # 导入开发板参数调整窗口
from .others_window import OtherParameterWindow  # 导入其他参数设置窗口
from .model_window import ModelParameterWindow  # 导入模型参数窗口
from .camera_window import CameraParameterWindow #导入摄像头参数窗口
...
class MainWindow(QMainWindow, Ui_Form):
    ...
    def open_DemoBoard_window(self):
        ...
        if not self.DemoBoard_window:  # 避免重复创建
            self.DemoBoard_window = DemoBoardParameterWindow()
    ...
    def open_others_window(self):
        ...
        if not self.others_window:  # 避免重复创建
            self.others_window = OtherParameterWindow()
    ...
    def open_model_window(self):
        ...
        if not self.model_window:  # 避免重复创建
            self.model_window = ModelParameterWindow()
            self.model_window.setWindowModality(Qt.NonModal)  # 非模态窗口
        self.model_window.show()    
    def open_camera_window(self):
        """打开摄像头参数窗口"""
        if not self.camera_window:  # 避免重复创建
            self.camera_window = CameraParameterWindow()
            self.camera_window.setWindowModality(Qt.NonModal)  # 非模态窗口
